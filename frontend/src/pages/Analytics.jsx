...
import { ArrowLeft, Users, Clock, Star, TrendingUp, Calendar, Trophy, Target, BarChart3, PieChart, Activity, Download, Filter, RefreshCw, Loader2, AlertCircle } from 'lucide-react';
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const analyticsResponse = {
          ...
          topScore: 98,
          medianScore: 78,
          passRate: 68, // percentage scoring above 70%
          attempts: Array(30).fill(null).map((_, i) => ({
            date: new Date(Date.now() - (29 - i) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            count: Math.floor(Math.random() * 10) + 2,
            averageScore: Math.floor(Math.random() * 30) + 60
          })),
          scoreDistribution: [
            { range: '0-20', count: 3, percentage: 2.4 },
            { range: '21-40', count: 8, percentage: 6.3 },
            { range: '41-60', count: 15, percentage: 11.8 },
            { range: '61-80', count: 52, percentage: 40.9 },
            { range: '81-100', count: 49, percentage: 38.6 }
          ],
          questionAnalysis: quizResponse.questions.map((q, i) => ({
            questionId: q.id,
            question: q.question,
            correctRate: Math.floor(Math.random() * 40) + 50,
            totalAttempts: 127,
            averageTime: Math.floor(Math.random() * 60) + 30
          })),
          recentAttempts: Array(20).fill(null).map((_, i) => ({
            username: `User${i + 1}`,
            score: Math.floor(Math.random() * 50) + 50,
            timeSpent: `${Math.floor(Math.random() * 10) + 5}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}`,
            completedAt: new Date(Date.now() - i * 3600000).toISOString(),
            rank: i + 1
          })),
          demographics: {
            byTimeOfDay: [
              { hour: '00-06', attempts: 5 },
              { hour: '06-12', attempts: 35 },
              { hour: '12-18', attempts: 52 },
              { hour: '18-24', attempts: 35 }
            ],
            byDayOfWeek: [
              { day: 'Monday', attempts: 18 },
              { day: 'Tuesday', attempts: 22 },
              { day: 'Wednesday', attempts: 15 },
              { day: 'Thursday', attempts: 19 },
              { day: 'Friday', attempts: 25 },
              { day: 'Saturday', attempts: 16 },
              { day: 'Sunday', attempts: 12 }
            ]
          }
        };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200 mb-8">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Question Performance Analysis</h3>
